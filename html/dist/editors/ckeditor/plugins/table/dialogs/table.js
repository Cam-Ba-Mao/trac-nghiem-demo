...
( function() {
    ...
    var colorDialog = CKEDITOR.plugins.colordialog;
        ...
        return {
            ...
            onOk: function() {
                ...
                if ( data.info ) {
                    ...
                    for(h = 0; h < table.$.rows.length; h++)
                    {
                        row = new CKEDITOR.dom.element(table.$.rows[h]);
                        for (m = 0; m < row.$.cells.length; m++){
                            newCell = new CKEDITOR.dom.element( row.$.cells[m]);
                            newCell.renameNode( 'td' );
                            newCell.removeAttribute( 'scope' );
                            if((h%2) == 0){
                                if(info.bg_row_odd){
                                    newCell.setStyle("background-color", info.bg_row_odd);
                                }else{
                                    newCell.removeStyle("background-color");
                                }
                            }else{
                                if(info.bg_row_even){
                                    newCell.setStyle("background-color", info.bg_row_even);
                                }else{
                                    newCell.removeStyle("background-color");
                                }
                            }
                    ...
                    if (table.$.tHead) {
                        if(info.bgColorHeader){
                            for (k = 0; k < table.$.tHead.rows.length; k++){
                                row = new CKEDITOR.dom.element( table.$.tHead.rows[ k ] );

                                for (m = 0; m < row.$.cells.length; m++){
                                    newCell = new CKEDITOR.dom.element( row.$.cells[m]);
                                    newCell.renameNode( 'td' );
                                    newCell.removeAttribute( 'scope' );
                                    newCell.setStyle("background-color", info.bgColorHeader)
                                }
                            }
                        }
                    }
                    ...
                    table.addClass("table_responsive");
                    console.log(table);
            ...
            contents: [ {
                ...
                elements: [ {
                    ...
                    },
                        {
                            ...
                            children: [ {
                                ...
                                children: [ {
                                    ...
                                    'default': editor.filter.check( 'table{width}' ) ? ( editable.getSize( 'width' ) < 500 ? '100%' : "100%" ) : 0,
                                ...
                                },
                                {
                                    ...
                                    'default': editor.filter.check( 'table[cellpadding]' ) ? 10 : 0,
                    ...
                    {
                        type: "hbox", widths: ['50%', '50%'], styles: ["vertical-align:top"], children: [{
                        type: 'hbox',
                        padding: 0,
                        widths: [ '60%', '40%' ],
                        children: [ {
                            type: 'text',
                            id: 'bg_row_odd',
                            requiredContent: "table{bg_row_odd}",
                            label: 'Màu dòng lẻ',
                            'default': '#f8f8f8',
                            onChange: function () {
                                var data = this.getDialog().getContentElement("advanced", "advStyles");
                                data && data.updateStyle("bg_row_odd", this.getValue())
                            },
                            setup: function (data) {
                                this.setValue(data.getAttribute("bg_row_odd") || "");
                                (data = data.getStyle("bg_row_odd"))
                            },
                            commit: function (data, selectedTable) {
                                var f = this.id;
                                data.info || (data.info = {});
                                data.info[f] = this.getValue();
                                this.getValue() ? selectedTable.setAttribute("bg_row_odd", this.getValue()) : selectedTable.removeAttribute("bg_row_odd");
                            }
                        },
                            colorDialog ? {
                                type: 'button',
                                ...
                                onClick: function() {
                                    editor.getColorFromDialog( function( color ) {
                                        if ( color )
                                            this.getDialog().getContentElement( 'info', 'bg_row_odd' ).setValue( color );
                                        this.focus();
                                    }, this );
                                }
                            } : spacer]
                        ...
                        children: [ {
                            type: 'text',
                            id: 'bg_row_even',
                            requiredContent: "table{bg_row_even}",
                            label: 'Màu dòng chẵn',
                            'default': '#ffffff',
                            onChange: function () {
                                var data = this.getDialog().getContentElement("advanced", "advStyles");
                                data && data.updateStyle("bg_row_even", this.getValue())
                            },
                            setup: function (data) {
                                this.setValue(data.getAttribute("bg_row_even") || "");
                                (data = data.getStyle("bg_row_even"))
                            },
                            commit: function (data, selectedTable) {
                                var f = this.id;
                                data.info || (data.info = {});
                                data.info[f] = this.getValue();
                                this.getValue() ? selectedTable.setAttribute("bg_row_even", this.getValue()) : selectedTable.removeAttribute("bg_row_even");
                            }
                        },
                            colorDialog ? {
                                type: 'button',
                                ...
                                onClick: function() {
                                    editor.getColorFromDialog( function( color ) {
                                        if ( color )
                                            this.getDialog().getContentElement( 'info', 'bg_row_even' ).setValue( color );
                                        this.focus();
                                    }, this );
                                }
                            } : spacer]
                    ...
                    },
                    {
                        ...
                        children: [ {
                            type: 'text',
                            id: 'bgColorHeader',
                            requiredContent: "table{bgColorHeader}",
                            label: 'Màu nền tiêu đề',
                            'default': '#eeeeee',
                            onChange: function () {
                                var data = this.getDialog().getContentElement("advanced", "advStyles");
                                data && data.updateStyle("bgColorHeader", this.getValue());

                            },
                            setup: function (selectedTable) {
                                this.setValue(selectedTable.getAttribute("bgColorHeader") || "");
                                (selectedTable = selectedTable.getStyle("bgColorHeader"));
                            },
                            commit: function (data, selectedTable) {
                                var f = this.id;
                                data.info || (data.info = {});
                                data.info[f] = this.getValue();
                                this.getValue() ? selectedTable.setAttribute("bgColorHeader", this.getValue()) : selectedTable.removeAttribute("bgColorHeader");
                            }
                        },
                            colorDialog ? {
                                type: 'button',
                                ...
                                onClick: function() {
                                    editor.getColorFromDialog( function( color ) {
                                        if ( color )
                                            this.getDialog().getContentElement( 'info', 'bgColorHeader' ).setValue( color );
                                        this.focus();
                                    }, this );
                                }
                            } : spacer ]
